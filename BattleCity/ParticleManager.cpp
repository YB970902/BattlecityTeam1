...
HRESULT ParticleManager::Init()
{
	...
	AddParticle(eParticleTag::BigBoom, eImageTag::EffectBigBoom, 4, 0.5, 0.25f);
...
void ParticleManager::AddParticle(eParticleTag particleTag, eImageTag imageTag, int frameX, float durationTime, float repeatTime)
{
	if (mMapParticleInfo.find(particleTag) != mMapParticleInfo.end()) { return; }
	mMapParticleInfo[particleTag] = PARTICLE_INFO{ imageTag, frameX, durationTime, repeatTime };
}

void ParticleManager::CreateParticle(eParticleTag tag, POINTFLOAT pos)
{
	Particle* newParticle = new Particle;
	newParticle->Init(IMG_MGR->FindImage(mMapParticleInfo[tag].ImageTag), mMapParticleInfo[tag].FrameX, mMapParticleInfo[tag].DurationTime, mMapParticleInfo[tag].RepeatTime);
